    ...
    protected function initDefaultButtons()
    {
        ...
        if(!isset($this->buttons['status'])){
            ...
            $this->buttons['status'] = function ($url,$model) {
                ...
                $url=['purchases/status','purchase_id'=>$model->purchase_id];
                $options = $this->deleteOptions;
                $title = Yii::t('kvgrid', 'Change Status');
                ...
                if ($this->_isDropdown) {
                    ...
                    $options['tabindex'] = '-1';
                    return '<li>' . Html::button('<i class="fa glyphicon glyphicon-edit"></i> Change status',[ 'value'=>Url::to(['purchases/status','purchase_id'=>$model->purchase_id]),'class'=>'btn btn-primary CreateModalButton'])  
                . '</li>' . PHP_EOL;
                } else {
                    return Html::button('<i class="fa glyphicon glyphicon-edit"></i> Change status',[ 'value'=>Url::to(['purchases/status','purchase_id'=>$model->purchase_id]),'class'=>'btn btn-primary CreateModalButton']);  
